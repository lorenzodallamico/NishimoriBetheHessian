...
* The directory ```data``` is empty. [Download from this link](https://mega.nz/folder/jh1QQAjQ#vT0f9c9sLTOSG1nEAxqGLg) the files ```featuresGAN.dat``` and ```featuresGAN_small.dat``` containing the features of 40k (resp. 6k) GAN images that can be used to test our algorithm and then put them in the folder ```data```.
